...
\beta_e, \beta_s &\sim Normal(0, 0.5) \\
\beta_t &\sim Normal(0, 0.15) \\
...
We conducted a simulation to demonstrate how we will conduct our analysis. We simulated values for $t$, $e$, $s$ (independent variables), and $f$ (dependent variable) according to our predictions, to demonstrate how we will perform our analysis. The full analysis will also include dependent variables $a$ (accessibility) and $b$ (biologging data).
...
h1_pred_2016 <- posterior_predict(biolog_mod,
                                  ...
                                  ndraws = 1000)
h1_data_2007 <- expand_grid(
  E = c("marine", "terrestrial"),
  S = c("aspatial", "spatial"),
  T = 2007
)
h1_pred_2007 <- posterior_predict(biolog_mod,
                                  newdata = h1_data_2007,
                                  ndraws = 1000)
...
h1_pred_2023 <- posterior_predict(biolog_mod,
                                  ...
                                  ndraws = 1000)
h1a_contrast <- (h1_pred_2023 - h1_pred_2007) %>% 
  t() %>% 
  cbind(select(h1_data_2007, E, S), .) %>% 
  pivot_longer(-c(E, S), names_to = "draw", values_to = "contrast")
h1b_contrast <- (h1_pred_2023 - h1_pred_2016) %>% 
  t() %>% 
...
mean_h1a <- mean(h1a_contrast$contrast) * 100
mean_h1b <- mean(h1b_contrast$contrast) * 100
...
### Plot contrasts
h1a_plot <- ggplot(h1a_contrast, aes(contrast)) +
  geom_bar(aes(y = after_stat(prop)), position = "dodge") +
  geom_vline(xintercept = mean(h1a_contrast$contrast), 
             color = "firebrick",
             linewidth = 2) +
  annotate("text", 
           x = mean(h1a_contrast$contrast) + 0.1, y = 0.75,
           label = sprintf("Mean = %0.2f", mean(h1a_contrast$contrast)),
           hjust = 0) +
  scale_y_continuous(limits = c(0, 1), labels = scales::percent) +
  labs(x = "H1a: Contrast of f (2023 vs 2007)",
       y = "Proportion") +
  theme_classic()

h1b_plot <- ggplot(h1b_contrast, aes(contrast)) +
  geom_bar(aes(y = after_stat(prop)), position = "dodge") +
  geom_vline(xintercept = mean(h1b_contrast$contrast), 
             color = "firebrick",
             linewidth = 2) +
  annotate("text", 
           x = mean(h1b_contrast$contrast) + 0.1, y = 0.75,
           label = sprintf("Mean = %0.2f", mean(h1b_contrast$contrast)),
           hjust = 0) +
  scale_y_continuous(limits = c(0, 1), labels = scales::percent) +
  labs(x = "H1b: Contrast of f (2023 vs 2016)",
       y = "Proportion") +
  theme_classic()
...
h2_plot <- ggplot(h2_contrast, aes(contrast)) +
  ...
  annotate("text", 
           ...
           hjust = 0) +
  scale_y_continuous(limits = c(0, 1), labels = scales::percent) +
  labs(x = "H2: Contrast of f (Terrestrial vs marine)",
       y = "Proportion") +
  theme_classic()
...
h3_plot <- ggplot(h3_contrast, aes(contrast)) +
  ...
  annotate("text", 
           ...
           hjust = 0) +
  scale_y_continuous(limits = c(0, 1), labels = scales::percent) +
  labs(x = "H3: Contrast of f (Spatial vs aspatial)",
       y = "Proportion") +
  theme_classic()

plot_grid(h1a_plot, h1b_plot, h2_plot, h3_plot, ncol = 2, labels = "AUTO")
...
Figure 2: Results of the open data assessment. Mean and standard error of dependent variables ($f$ findability, $a$ accessibility, $b$ biologging data) aggregated over **A** sensor type (aspatial or spatial), **B** ecosystem (marine or terrestrial), and **C** time (year of publication).

Figure 3: Model parameter estimates. Posterior distribution of parameters, as in [@fig-param] from the analysis of simulated data.

Figure 4: Hypothesis tests. Contrasts of posterior predictions for hypotheses H1, H2, and H3, as in [@fig-hypotheses] from the analysis of simulated data.
