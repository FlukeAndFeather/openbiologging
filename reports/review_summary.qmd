...
novel_data_sources <- readRDS(here::here("outputs/novel_data_sources.rds"))
...
Summaries of reviewed publications by year, source, sensors, and taxa.
...
Summaries of FAIR scores for biologging and non-biologging data.

```{r}
#| label: data-aggregate

data_by_paper <- novel_data_sources %>% 
  group_by(id, is_biologging) %>%
  summarize(across(c(availability_statement, F1:Complete), any),
            .groups = "drop")

data_by_year <- data_by_paper %>% 
  left_join(novel_data_papers, by = "id") %>% 
  group_by(year, is_biologging) %>% 
  filter(year >= 2007) %>% 
  summarize(across(
    availability_statement:Complete, 
    list(n = \(x, ...) sum(!is.na(x)),
         rate = \(x, ...) sum(x, na.rm = TRUE) / sum(!is.na(x)))
  ), .groups = "drop") %>%
  pivot_longer(-c(year, is_biologging),
               names_to = c("var", "stat"),
               names_pattern = "(.*)_(.*)",
               values_to = "val") %>% 
...
ggplot(data_by_year,
       aes(x = year, y = rate, color = is_biologging)) +
